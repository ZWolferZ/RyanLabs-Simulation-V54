...
void HelloGL::Display(void) const
{
	...
	DrawString("CONTROLS:", &c, -1.0f, -0.4f);

	DrawString("Move mouse to rotate camera around the origin", &c, -1.0f, -0.5f);
	DrawString("Scroll mouse wheel to zoom in/out", &c, -1.0f, -0.55f);
	DrawString("WASD / ARROWS to move camera position", &c, -1.0f, -0.60f);
	DrawString("Press 'R' to reset camera", &c, -1.0f, -0.65f);

	DrawString("Press 'F' to toggle fullscreen", &c, -1.0f, -0.7f);
...
void HelloGL::updateRefreshrate()
{
	glutPostRedisplay();
...
void HelloGL::keyboardControls(const unsigned char key, int x, int y)
{
	switch (key)
	{
	case 'r': // Reset camera
...
void HelloGL::keyboardControlsUp(const unsigned char key, int x, int y)
{
...
void HelloGL::InitObjects()
{
	...
	if (toggle == false)
	{
		...
		//	auto* OBJMesh = OBJLoader::LoadOBJ(const_cast<char*>("Objects/teapot.obj"));
...
void HelloGL::DrawString(const char* text, Color* color, float x, float y)
{
...
void HelloGL::mouseRotation(int x, int y)
{
	if (fullScreen)
	{
		if (!mouseToggle)
